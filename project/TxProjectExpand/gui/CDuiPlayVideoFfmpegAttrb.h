...
public :
	...
	{
		CString lc_cs;
		this->p_label_file_BiTeLv->SetText(_T("δ֪"));
		this->p_label_file_ShiChang->SetText(_T("δ֪"));
		this->p_re_file_MetaData->SetText(_T("δ֪"));
		this->p_label_video_BianMaFangShi->SetText(_T("δ֪"));
		this->p_label_video_ZhenLv->SetText(_T("δ֪"));
		this->p_label_video_HuaMianDaXiao->SetText(_T("δ֪"));
		this->p_label_audio_CaiYangLv->SetText(_T("δ֪"));
		this->p_label_audio_BianMaFangShi->SetText(_T("δ֪"));
		this->p_label_audio_ShengDaoShu->SetText(_T("δ֪"));
		{
			this->p_label_file_FengZhuangGeShi->SetText(_T("δ֪"));
		...
		if(_iVideoIdx>=0&&_pFormatCtx->streams[_iVideoIdx]->codec!=NULL)
		{
			AVStream *pAvStream = _pFormatCtx->streams[_iVideoIdx];
			AVCodecContext *pCodecCtx = _pFormatCtx->streams[_iVideoIdx]->codec;
		...
		if(_iAudioIdx>=0&&_pFormatCtx->streams[_iAudioIdx]->codec!=NULL)
		{
			AVStream *pAvStream = _pFormatCtx->streams[_iAudioIdx];
			AVCodecContext *pCodecCtx = _pFormatCtx->streams[_iAudioIdx]->codec;
			{
