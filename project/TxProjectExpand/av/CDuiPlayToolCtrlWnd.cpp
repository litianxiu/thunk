...
void CDuiPlayToolCtrlWnd::InitWindow()
{
	...
	if(this->bInitControlUI==FALSE)
	{
		...
		this->buildSaftWidgetPtr(&this->p_sliderVol,_T("sliderVol"));
...
void CDuiPlayToolCtrlWnd::Notify(DuiLib::TNotifyUI& _msg)
{
	...
	if(_msg.sType.Compare(DUI_MSGTYPE_CLICK)==0)
	{
		...
		{
			if(this->p_cb_func->clickSetVolume(0))
			{
		...
		{
			if(this->p_cb_func->clickSetVolume(this->fVolumeRatio))
			{
	...
	else if(_msg.sType.Compare(DUI_MSGTYPE_VALUECHANGED)==0)
	{
		...
		else if(_msg.pSender==p_sliderVol)
		{
			int x1=(int)p_sliderVol->GetMinValue();
			int x2=(int)p_sliderVol->GetMaxValue();
			int xxx=(int)p_sliderVol->GetValue();
			if(x1<=xxx&&xxx<=x2&&x1<x2)
			{
				this->p_cb_func->clickSetVolume((float)(xxx-x1)/(float)(x2-x1));
			}
		}
	}
